...
# 填入你注册的硅基流动或DeepSeek密钥。
# 如果不填，代码会自动降级为漂亮的蓝色标准快讯卡片，绝不报错。
...
SITE_LIST = [
    ...
    {"name": "雪球投资热帖", "url": "https://rsshub.app/xueqiu/hots"},
...
def get_ai_summary(title, content):
    ...
    请必须严格按照以下格式和行数回复（不要说任何客套话，不要附带多余符号）：
